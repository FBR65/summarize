...
def llm(model):
    match model:
        ...
        case "Qwen Coder":
            return "acidtib/qwen2.5-coder-cline:7b"
        case "Nemo":
            return "maryasov/mistral-nemo-cline:12b-instruct-2407-q2_K "
        case "Deepseek-R1":
            return "tom_himanen/deepseek-r1-roo-cline-tools:8b"


async def user(user_message, history):
    # Add the user message to the history in the correct format
    history.append({"role": "user", "content": user_message})
    return "", history


async def bot(msg, history, t, system_message):
    chat_history = ChatHistory()
    # Reconstruct the chat history for async_bot
    ...
    partial_text = ""
    async for text_chunk in async_bot_stream(
        user_message, chat_history, system_message_obj, t
    ):
        ...
        with gr.Column(scale=1):
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    senden = (
                        senden.click(llm, db, t)
                        .then(user, [msg, chatbot], [msg, chatbot], queue=True)
                        .then(bot, [msg, chatbot, t, system_message], chatbot)
                    )
                    enter_submit = (
                        msg.submit(llm, db, t)
                        .then(user, [msg, chatbot], [msg, chatbot], queue=True)
...
# Encapsulate (runs only, when this file is called from terminal)
